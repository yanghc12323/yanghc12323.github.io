---
title:          "The Emotional Dynamics of AI-Patient Communication: Effects of Gender and Consultation Stage"
title_zh:       "AI与患者沟通中的情感动态：性别与诊疗阶段的影响"
date:           2026-03-15 00:01:00 +0800
selected:       true
pub:            "HCII 2026 (Human-Computer Interaction International)"
pub_date:       "2026 (Full text paper)"
abstract: >-
  This study investigates how AI-driven medical assistants provide emotional support to patients across pre-, mid-, and post-consultation stages, and examines the moderating role of gender.
  A 2×3×2 mixed factorial experiment (N=32) was conducted using a custom GPT-4o-powered AI doctor prototype (Doc.Tsing), with extended PAD emotion scales and multidimensional trust scales.
  Results revealed a significant three-way interaction effect of gender × consultation stage on perceived control, with males experiencing a notable drop in control during mid-consultation.
  Based on empirical findings, we propose "context-aware" and "personalized communication" design strategies for adaptive medical AI systems.
abstract_zh: >-
  本研究探讨了AI驱动的医疗助手在诊前、诊中、诊后不同阶段如何为患者提供情感支持，并考察了性别的调节作用。
  通过自主开发的基于GPT-4o的AI医生原型（Doc.Tsing），采用混合因子实验设计（N=32），运用扩展PAD情绪量表和多元信任量表进行测量。
  实验结果揭示了性别×问诊阶段在感知控制上的显著三阶交互效应——男性在问诊中期感知控制显著下降。
  基于实证发现，我们提出了面向自适应医疗AI系统的"情境感知"和"个性化沟通"设计策略。
cover:          /assets/images/covers/HCII2026.png
authors:
  - Haochen Yang
  - Mingxiao Li
  - Zhuoran Gao
  - Ruohan Yang
  - Pei-Luen Patrick Rau*
links:
  Paper: https://doi.org/10.1007/978-3-032-29903-1_32
---