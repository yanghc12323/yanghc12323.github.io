...
abstract: >-
  ...
  Using generalized linear modeling (GLM) and multivariate Granger causality analysis, we validated the "neural efficiency hypothesis," demonstrating that semantic consistency significantly reduces activation in the posterior parietal cortex (PPC).
...
abstract_zh: >-
  本研究利用功能性近红外光谱（fNIRS）技术，探索视觉色彩饱和度与振动触觉强度之间跨模态一致性的神经机制。
  设计了2×2因子跨模态实验，并基于iOS应用（Objective-C + Core Haptics）实现精确同步的视觉色彩刺激与毫秒级振动触觉反馈。
  通过广义线性模型（GLM）和多元Granger因果分析，验证了"神经效率假说"，证明语义一致性显著降低了后顶叶皮层（PPC）的激活程度。
  研究结果为多感觉整合中的除法归一化机制提供了神经生理学证据，并为多模态HCI中的跨模态反馈设计提供了自下而上的神经科学指导。
